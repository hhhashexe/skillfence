...
// === v2.1 PATTERNS — From AgentPass deep audit findings ===
const V21_PATTERNS = [
  // Default/Hardcoded Secrets
  { id: 'SECRET-001', re: /(?:secret|password|key)\s*[:=]\s*["'][^"']*(?:DO.?NOT.?USE|changeme|default|example|placeholder|insecure|dummy|temp)[^"']*["']/gi, sev: 'CRITICAL', cat: 'credential', desc: 'Default/placeholder secret in code — change before deploy', owasp: 'MCP-03', fix: 'Use environment variables, never hardcode secrets' },
  { id: 'SECRET-002', re: /(?:fallback|default)\s*(?:=|:)\s*["'][a-zA-Z0-9_-]{16,}["']/gi, sev: 'HIGH', cat: 'credential', desc: 'Hardcoded fallback secret — remove before production', owasp: 'MCP-03', fix: 'Require secrets via env vars, fail fast if missing' },
  
  // Registration/Auth Patterns
  { id: 'REG-001', re: /verified\s*[:=]\s*false.*(?:INSERT|create)|(?:INSERT|create).*verified\s*[:=]\s*false/gi, sev: 'MEDIUM', cat: 'auth', desc: 'Account created with verified=false but may grant access', owasp: 'MCP-08', fix: 'Require email verification before granting API access' },
  { id: 'REG-002', re: /Email already registered|email.*exists|EMAIL_EXISTS/gi, sev: 'MEDIUM', cat: 'auth', desc: 'Account enumeration via registration response', owasp: 'MCP-08', fix: 'Return identical response for existing and new emails' },
  
  // Password Policy
  { id: 'PASS-001', re: /password.*\.min\(\s*[1-7]\s*\)|min.*password.*[1-7]|password.*length\s*[<>=]+\s*[1-7]/gi, sev: 'HIGH', cat: 'auth', desc: 'Weak password minimum length (< 8 chars)', owasp: 'MCP-03', fix: 'Require minimum 12 characters with complexity rules' },

  // Unhandled Errors / Information Disclosure
  { id: 'ERR-001', re: /Internal server error|INTERNAL_ERROR/gi, sev: 'MEDIUM', cat: 'info_disclosure', desc: 'Generic 500 error — may leak stack traces in debug', owasp: 'MCP-04', fix: 'Implement global error handler, sanitize error responses' },
  
  // Open Registration
  { id: 'OPEN-REG-001', re: /\/auth\/register|\/signup.*POST|register.*route/gi, sev: 'LOW', cat: 'auth', desc: 'Open registration endpoint — verify rate limiting exists', owasp: 'MCP-08', fix: 'Add captcha, email verification, and strict rate limiting' },
];

PATTERNS.push(...V21_PATTERNS);

// === v2.2 PATTERNS — From AgentPass Round 4 deep audit ===
const V22_PATTERNS = [
  // Trust/Reputation Manipulation
  { id: 'TRUST-001', re: /(?:trust|reputation|score|verified).*(?:self|own|owner).*(?:set|update|patch)|(?:owner|self).*(?:verify|trust_score|verified)/gi, sev: 'HIGH', cat: 'logic', desc: 'Self-verification of trust/reputation — owner can set own trust flags', owasp: 'MCP-08', fix: 'Require external verification or admin approval for trust flags' },
  
  // Unauthenticated Notification Endpoints
  { id: 'NOTIFY-001', re: /(?:notifications?|messages?)\/[:*](?:address|email|phone|user)/gi, sev: 'HIGH', cat: 'auth', desc: 'Notification endpoint with user parameter — verify auth is required', owasp: 'MCP-03', fix: 'Require authentication and scope to authenticated user only' },
  
  // Static HKDF/KDF Salt
  { id: 'CRYPTO-003', re: /HKDF_SALT\s*=\s*["'][^"']+["']|hkdf\([^)]*["'][a-z-]+["']/gi, sev: 'MEDIUM', cat: 'crypto', desc: 'Static HKDF salt — should use per-operation random salt', owasp: 'MCP-03', fix: 'Generate random salt per operation, store alongside ciphertext' },
];

PATTERNS.push(...V22_PATTERNS);
